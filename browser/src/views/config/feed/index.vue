    ...
    <div class="">
        <div class="demoTable">
            <el-form :inline="true" :model="tableQuery" class="demo-form-inline" ref="tableQuery">
                <el-form-item label="饲料名称：" prop="name">
                    <el-input v-model.trim="tableQuery.name" placeholder="请输入饲料名称"></el-input>
                </el-form-item>
        ...
        <div class="demoTable">
            <el-table
                ...
                <el-table-column prop="create_time" label="创建时间" :formatter="dateFormat" width="150"></el-table-column>
        ...
        <el-dialog :visible.sync="dialogCreateFormVisible" :modal-append-to-body='false'>
            <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="120px" style = "margin-top:20px">
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="饲料名称：" prop="name">
                            <el-input v-model.trim="temp.name" placeholder="请输入饲料名称"></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item label="生产企业：" prop="producer">
                            <el-input v-model.trim="temp.producer" placeholder="请输入生产企业"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="备注：" prop="remask">
                            <el-input type="textarea" v-model.trim="temp.remark" placeholder="请输入备注" ></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                rules:{
                    name: [{ required: true, message: '请输入饲料名称', trigger: 'blur' }],
                    producer: [{ required: true, message: '请输入生产企业', trigger: 'blur' }],
        ...
        methods: {
            ...
            dateFormat:function(row, column) {
                ...
                return util.formatTime(new Date(date),'yyyy-MM-dd hh:mm:ss');
            },
            //清空对象所有属性
            resetTemp(temp){
            ...
            fetchData(current) {
                ...
                params.append("producer",this.tableQuery.producer);

                this.$http({
                    method: "post",
                    url: "/web-server/config/feed/feed.json?action=query",
                    data: params
                }).then(res => {
            ...
            createData() {
                this.$refs['dataForm'].validate((valid) => {
                    ...
                    let params = new URLSearchParams();
                    params.append("name", this.temp.name.trim());
                    params.append("producer", this.temp.producer.trim());
                    params.append("batchNumber", this.temp.batchNumber.trim());
                    params.append("specs", this.temp.specs.trim());
                    params.append("unit", this.temp.unit.trim());
                    params.append("price", this.temp.price);
                    params.append("remark", this.temp.remark.trim());

                    this.$http({
                        method: "post",
                        url: "/web-server/config/feed/feed.json?action=save",
                        data: params
                    }).then(res => {
            ...
            handleEdit(id){
                ...
                this.$http({
                    method: "post",
                    url: "/web-server/config/feed/feed.json?action=getFeed",
                    data: params
                }).then(res => {
            ...
            removeBatch(rows) {
                ...
                this.$confirm('确定要删除选中的行吗?', '提示').then(() => {
                    this.$http({
                        method: "post",
                        url: "/web-server/config/feed/feed.json?action=delete",
                        data: params
                    }).then(res => {
						this.fetchData();
